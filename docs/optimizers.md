...
$$
g_k=\nabla f(x_k+\mu v_k),\qquad
v_{k+1}=\mu v_k-\alpha g_k,\qquad
x_{k+1}=x_k+v_{k+1}
$$
...
x_{t+1}=x_t-\alpha\frac{g_t}{\sqrt{v_t}+\varepsilon}
$$
...
x_{t+1}=x_t-\alpha\frac{g_t}{\sqrt{s_t}+\varepsilon}
$$
...
$$
\alpha_k=\alpha_0\gamma^{\left\lfloor k / s \right\rfloor}
$$
...
$$
\alpha_k=\alpha_{\min}
+\frac{1}{2}(\alpha_0-\alpha_{\min})\left(1+\cos\frac{\pi k}{T}\right)
$$

Warmup применяется поверх выбранного расписания:

$$
\alpha_k\leftarrow \alpha_k\frac{k+1}{w},\qquad k<w
$$
...
$$
\left|\nabla f(x+\alpha p)^Tp\right|\le c_2\left|\nabla f(x)^Tp\right|
$$
...
$$
Hp=-\nabla f(x),\qquad x_{\text{next}}=x+\alpha p
$$
...
BFGS хранит плотное приближение обратного гессиана $B\approx H^{-1}$:

$$
s=x_{\text{next}}-x,\qquad
y=\nabla f(x_{\text{next}})-\nabla f(x),\qquad
\rho=\frac{1}{y^Ts}
$$

$$
B_{\text{next}}=(I-\rho sy^T)B(I-\rho ys^T)+\rho ss^T
$$
